...
namespace PCG.Terrain
{
    ...
    public class Chunk2DMeshBuilder : Chunk2DGenerationStage
    {
        ...
        {
            world = generator.GetComponent<ChunkWorld2D>();
            
            int indicesCount = (world.chunkResolution.x - 1) * (world.chunkResolution.y - 1) * 6;
            ...
            JobHandle handle = CalculateTriangles(world.chunkResolution, _triangles);

            int vertexCount = world.chunkResolution.x * world.chunkResolution.y;
            NativeArray<float2> _uvs =
                new NativeArray<float2>(vertexCount, Allocator.TempJob, NativeArrayOptions.UninitializedMemory);
            handle = CalculateUVs(world.chunkResolution, _uvs, handle);
        ...
        public override IEnumerator Generate(Chunk2D chunk)
        {
            ...
            Vector3 meshScale = Vector3.one * world.chunkWorldSize / (world.chunkResolution.x - 1);
            meshScale.y = world.worldHeight;
            
            var mesh = chunk.filter.mesh;
            mesh.Clear();

            var vertices = new NativeArray<float3>(world.chunkArraySize, Allocator.Persistent,
            ...
            int2 chunkSize = chunk.world.chunkResolution;

            // Vertices
            mesh.SetVertices(vertices, 0, vertices.Length, flags);
            if (vertexNormals && !recalculateNormals)
                mesh.SetNormals(chunk.GetContent<ChunkNormals2D>().content, 0, world.chunkArraySize);
